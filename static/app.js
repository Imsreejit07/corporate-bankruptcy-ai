...
function renderFields() {
  fieldContainer.innerHTML = FEATURES.map(([code, label]) => `
  ...
  chart.innerHTML = contributions.map((item) => {
    ...
    return `
      <div class="contribution-row">
        <span class="contribution-name" title="${escapeHtml(item.label)}">${escapeHtml(item.feature)} · ${escapeHtml(item.label)}</span>
        <span class="contribution-visual">
          <span class="contribution-bar ${polarity}" style="width:${width}%"></span>
        </span>
        <span class="contribution-number">${formatShap(item.shap_value)}</span>
      </div>
    `;
  }).join("");
  ...
  list.innerHTML = contributions.slice(0, 5).map((item, index) => {
    ...
    return `
      <div class="explanation-item">
        ...
        <span><strong>${escapeHtml(item.label)} (${escapeHtml(item.feature)})</strong> ${direction}. Input value: ${item.input_value}.</span>
      </div>
    `;
  }).join("");
}
